...
const Getsalecars = () => {
  ...
  const getallsalecars = async () =>{
    try {
        const res = await fetch ('/getAvailableSaleCars', {
            method: 'GET',
        });
...
let carIdFromDashBoard;
  const deleteUser = (e) =>{
    ...
    return fetch("/deleteSaleCarFromDashboard", {
      method: "POST",
      headers:{
  ...
  <section className="home-section">
    ...
    <div className = "salecartableDiv">
            ...
            <h1 className="heading"><span>Available Sale Cars</span></h1>
        ...
        {getCars.map((getCars) => 
            <tbody className = "carDiv"  key={getCars._id}>
